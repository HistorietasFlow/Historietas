...
/**
 * Pagina consultas que precisam filtrar uma lista grande de ids. Cada lote
 * recebe ranges próprios para evitar tanto o limite de linhas quanto URLs de
 * consulta excessivamente longas no Data API.
 *
 * @template TRegistro, TItem
 * @param {{
 *   nomeColecao: string;
 *   itens: TItem[];
 *   buscarPaginaLote: (
 ...
 *     pagina: number,
 *     indiceLote: number,
 *   ) => Promise<{ data: TRegistro[] | null; error: unknown }>;
 *   tamanhoLote?: number;
 *   tamanhoPagina?: number;
 *   maximoPaginas?: number;
 * }} opcoes
 * @returns {Promise<TRegistro[]>}
 */
export async function carregarTodasPaginasPorLotesSupabase({
  nomeColecao,
  itens,
  buscarPaginaLote,
  tamanhoLote = 100,
  ...
  if (typeof buscarPaginaLote !== "function") {
    throw new TypeError("buscarPaginaLote deve ser uma função.");
  }

  const lotes = dividirEmLotesSupabase(itens, tamanhoLote);
  const registros = [];

  for (let indiceLote = 0; indiceLote < lotes.length; indiceLote += 1) {
    const lote = lotes[indiceLote];
    const registrosLote = await carregarTodasPaginasSupabase({
      nomeColecao: `${nomeColecao} (lote ${indiceLote + 1})`,
      tamanhoPagina,
      maximoPaginas,
      buscarPagina: (inicio, fim, pagina) =>
        buscarPaginaLote(lote, inicio, fim, pagina, indiceLote),
    });

    registros.push(...registrosLote);
  }
