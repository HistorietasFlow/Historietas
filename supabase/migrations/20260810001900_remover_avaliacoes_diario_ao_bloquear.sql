...
where exists (
  ...
  where (
    bloqueio.bloqueador_id = avaliacao.avaliador_id
    and bloqueio.bloqueado_id = avaliacao.diario_user_id
  )
  or (
    bloqueio.bloqueador_id = avaliacao.diario_user_id
    and bloqueio.bloqueado_id = avaliacao.avaliador_id
  )
);
...
begin
  ...
  delete from public.diario_avaliacoes avaliacao
  where (
    avaliacao.avaliador_id = v_bloqueador_id
    and avaliacao.diario_user_id = p_bloqueado_id
  )
  or (
    avaliacao.avaliador_id = p_bloqueado_id
    and avaliacao.diario_user_id = v_bloqueador_id
  );
