-- 20260728000600_denuncias_obras_capitulos_comentarios.sql
-- Amplia o sistema seguro de denúncias para:
--   obra, capitulo e comentario_obra.
--
-- Pré-requisitos:
...
begin
  ...
  loop
    if to_regclass(format('public.%I', v_tabela)) is null then
      raise exception 'A tabela public.% precisa existir antes desta migration.', v_tabela;
    end if;
  end loop;
  ...
  loop
    if not exists (
      ...
      where table_schema = 'public'
        and table_name = 'comunidade_denuncias'
        and column_name = v_coluna
    ) then
      raise exception 'A coluna public.comunidade_denuncias.% não existe.', v_coluna;
    end if;
  end loop;
...
begin
  if tg_op = 'INSERT' then
    ...
    if new.alvo_tipo = 'post' then
      ...
      if not found then
        raise exception 'A obra denunciada não existe ou não está disponível.'
          using errcode = 'P0002';
      end if;
      ...
      if not found then
        raise exception 'O capítulo denunciado não existe ou não está disponível.'
          using errcode = 'P0002';
      end if;

    else
      select comentario_obra.user_id
      into v_autor_alvo
      from public.comentarios_obras comentario_obra
      inner join public.obras obra
        on obra.id = comentario_obra.obra_id
      where comentario_obra.id = v_alvo_uuid
        and coalesce(obra.publicado, false) = true
      limit 1;

      if not found then
        raise exception 'O comentário da obra denunciado não existe ou não está disponível.'
          using errcode = 'P0002';
      end if;
    end if;
...
comment on function public.validar_comunidade_denuncia() is
  'Valida denúncias de Comunidade, obras, capítulos e comentários, incluindo autoria, visibilidade e limite de envio.';
...
comment on function public.criar_denuncia(text, uuid, text, text) is
  'Cria denúncias seguras para Comunidade, obras, capítulos e comentários usando auth.uid().';
