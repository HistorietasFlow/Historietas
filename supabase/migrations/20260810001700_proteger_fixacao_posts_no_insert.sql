...
begin
  if tg_op = 'INSERT' then
    if coalesce(new.fixado, false) = true
      or new.fixado_em is not null
      or new.fixado_por is not null
    then
      if auth.uid() is null
        or not public.comunidade_usuario_e_admin()
      then
        raise exception 'Somente administradores podem fixar posts.'
          using errcode = '42501';
      end if;
...
drop trigger if exists trigger_atualizar_fixado_comunidade
...
before insert or update of fixado, fixado_em, fixado_por
on public.comunidade_posts
for each row
execute function public.definir_fixacao_comunidade_post();

drop function if exists public.atualizar_fixado_comunidade();
