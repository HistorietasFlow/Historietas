...
test("Home carrega navegação e busca filtra obras", async ({ page }) => {
  ...
  await expect(page.locator('a[href^="/obra/"]').first()).toBeVisible({ timeout: 20_000 });

  await search.fill("__historietas_e2e_sem_resultado_9f8d7c__");
  await expect(page.getByText("Nenhuma obra encontrada", { exact: true }).first()).toBeVisible();
...
test("Explorar aplica categoria, abre filtros e limpa estado", async ({ page }) => {
  const runtime = monitorRuntime(page);
  ...
  await expect(modal.getByRole("button", { name: /^Mais recentes/ })).toContainText("✓");
...
test("Home e Explorar não criam rolagem horizontal no celular", async ({ page }) => {
  ...
  for (const route of ["/", "/explorar"]) {
    ...
    expect(dimensions.document, `Rolagem horizontal detectada em ${route}`).toBeLessThanOrEqual(
